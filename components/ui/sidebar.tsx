...
type SidebarContextProps = {
  ...
  pinned: boolean;
  togglePin: () => void;
  setHoverExpanded: (hovered: boolean) => void;
  openMobile: boolean;
...
function SidebarProvider({
  ...
  // Pinned reflects the user's explicit intent (toggle button).
  // Hover-expanded is a transient overlay state when not pinned.
  // Effective open = pinned || hoverExpanded.
  const [_pinned, _setPinned] = React.useState(openProp ?? defaultOpen);
  const [hoverExpanded, setHoverExpanded] = React.useState(false);
  const pinned = openProp ?? _pinned;
  const open = pinned || hoverExpanded;
  ...
  // Helper to toggle the sidebar (mobile uses sheet, desktop pins).
  const toggleSidebar = React.useCallback(() => {
    return isMobile ? setOpenMobile((open) => !open) : togglePin();
  ...
  const contextValue = React.useMemo<SidebarContextProps>(
    ...
    [state, open, setOpen, pinned, togglePin, isMobile, openMobile, setOpenMobile, toggleSidebar],
  );
    ...
    >
      ...
      <div
        ...
        className={cn(
          ...
          // Hover-peek mode: when not pinned, gap stays at icon width regardless of hover (so content doesn't shift)
          variant === 'floating' || variant === 'inset'
      ...
      <div
        ...
        className={cn(
          ...
          // Overlay shadow when hover-expanded (not pinned, but currently showing)
          'group-data-[pinned=false]:group-data-[state=expanded]:shadow-2xl',
          className,
        )}
        {...props}
      >
...
function SidebarTrigger({ className, onClick, ...props }: React.ComponentProps<typeof Button>) {
  ...
  return (
    <Button
      ...
      {...props}
    >
      <ChevronLeftIcon className={cn('transition-transform duration-200', state === 'collapsed' && 'rotate-180')} />
